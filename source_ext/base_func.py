...
async def command_help(bot, message):
    emb = discord.Embed(title='깔롤랭은 국룰입니다',description=Docs.help)
    await message.channel.send(embed = emb)

async def command_choice(bot, message):
...
async def command_잊어(bot, message):#샤키야 key커맨드
...
async def command_배워(bot, message) :#샤키야 key커맨드 value커맨드
...
async def command_사진(bot, message):
    ...
    if image is None:
        ...
        em = discord.Embed(title = "%s의 이미지 검색결과" %findg)
        em.set_image(url = image)
    await message.channel.send(embed = em)
    
    
        
async def command_사전(bot, message):
    ...
    if findit is None:
        ...
        em2 = discord.Embed(title = "%s의 네이버사전검색 결과" % findn,description = "%s" %findit)
        await message.channel.send(embed = em2)
        
async def command_굴러(bot, message : discord.message):
    try:
        await message.channel.send(random.choice(["데구르르 꽝","꽝 데구르르","데구르르 뎅강","ㄷㄱㄹㄹ ㄷㄱ","야랄,,, 너나 구르세요"]))
    except:
        pass

async def command_Hello(bot, message):
