...
class base_func:
    ...
    async def command_잊어(self, bot, message):#샤키야 key커맨드
    ...
    async def command_배워(self, bot, message) :#샤키야 key커맨드 value커맨드
    ...
    async def command_사진(self, bot, message):
    ...
    async def command_사전(self, bot, message):
    ...
    @staticmethod
    async def command_굴러(self, bot = None, message: discord.message =  None ):
        await message.channel.send(random.choice(["데구르르 꽝","꽝 데구르르","데구르르 뎅강","ㄷㄱㄹㄹ ㄷㄱ","야랄,,, 너나 구르세요"]))
    
    async def command_Hello(self, bot, message):
